...
{% if var(passthrough_var_name, []) != [] and var('rag_hubspot_property_enabled', True) %}
  {% set source_name = passthrough_var_name.replace('rag_hubspot__', '').replace('_pass_through_columns', '') %}
  {%- set col_list = var(passthrough_var_name) -%}

  {%- for col in col_list -%} -- Create label cols
    {%- if col.add_property_label or var('rag_hubspot__enable_all_property_labels', false) -%}
